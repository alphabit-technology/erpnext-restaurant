PayForm = class PayForm {
    ...
    make_payment_button(){
        this.button_payment = new JSHtml({
            ...
            content: `<span style="font-size: 26px; font-weight: 400">{{text}} ${this.order.total_money()}</span>`,
            text: `${__("Pay")}`
        }).on("click", (obj, input) => {
                if(!RM.can_pay()) return;
                //if(!RM.permissions.pay) return;
        ...
        setTimeout(()=>{
            ...
            //if(!RM.permissions.pay) this.button_payment.disable();
        }, 0)
    }
        ...
        CETI.api.call({
            ...
            always: (r) => {
                ...
                if(typeof r.message != "undefined" && r.message.status) {
                    ...
                    this.order.order_manage.empty_inputs();
    ...
    print(invoice_nane){
        if(!RM.can_pay) return;
		window.open(`printview?doctype=Sales%20Invoice&name=${invoice_nane}&trigger_print=1&format=${RM.pos_profile.print_format_for_online}&no_letterhead=0`, '_blank');
	}
