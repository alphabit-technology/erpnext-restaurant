class ProductItem{
	...
	get_item_html(item) {
		...
		return new JSHtml({
			...
			if(typeof this.order_manage.current_order != "undefined"){
				...
				//if(this.order_manage.current_order.data.owner !== frappe.session.user) return;
				...
				this.set_items_detail(_item).then((data) => {
					//console.log(data);
					data["identifier"] = _item["identifier"];
					data["status"] = _item["status"];
					data["notes"] = _item["notes"];
					this.order_manage.current_order.queue_item(data);

					/*this.work_station = data.work_station;
					this.pos_profile = data.pos_profile;*/
				});

			}
			//self.order_manage.add_from_products(_item, "Add");
		}).html()
	...
	set_items_detail(item) {
		return new Promise(res => {
			frappe.call({
				method: 'erpnext.stock.get_item_details.get_item_details',// this.url_manage + "get_work_station",
				freeze: true,
				args: {args: item}
			}).then(r => {
				res(r.message);
